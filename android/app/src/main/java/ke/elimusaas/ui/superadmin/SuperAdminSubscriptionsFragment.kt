...
import ke.elimusaas.data.SubscriptionItem
...
class SuperAdminSubscriptionsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, s: Bundle?) {
        ...
        scope.launch {
            val subs: List<SubscriptionItem> = withContext(Dispatchers.IO) {
                try { ApiClient(requireContext()).getSubscriptions() }
                catch (e: Exception) { emptyList<ke.elimusaas.data.SubscriptionItem>() }
            }
            if (!isAdded) return@launch
            if (subs.isEmpty()) {
                container?.addView(TextView(context).apply {
                    text = "No subscriptions found"
                    textSize = 14f; setPadding(16, 64, 16, 32)
            ...
            subs.take(30).forEach { sub ->
                ...
                row.findViewById<TextView>(R.id.tvStudentGrade)?.apply {
                    text = sub.status.uppercase()
                    setTextColor(if (sub.status == "active") Color.parseColor("#1B5E20") else Color.parseColor("#B71C1C"))
                }
