...
import ke.elimusaas.utils.SecurityHelper
...
class LoginActivity : AppCompatActivity() {
    ...
    private fun doLogin(
        ...
        // Rate limiting check
        val (locked, minutesLeft) = SecurityHelper.isLockedOut(this)
        if (locked) {
            tvError.text = "Too many failed attempts. Try again in $minutesLeft minute(s)."
        ...
        // Capture context reference before entering coroutine
        val ctx = this
        ...
        scope.launch {
            val r = withContext(Dispatchers.IO) {
                try {
                    ...
                    val msg = "Network error: " + (e.message ?: "Cannot reach server")
                    ke.elimusaas.data.LoginResponse(null, null, null, msg)
                }
            }
            setLoading(btn, progress, false)
            when {
                r.error != null -> {
                    SecurityHelper.recordFailedAttempt(ctx)
                    tvError.text = r.error
                    tvError.visibility = View.VISIBLE
                }
                r.accessToken != null && r.user != null -> {
                    SecurityHelper.resetAttempts(ctx)
            ...
            .setPositiveButton("Send Reset Link") { _, _ ->
                ...
                if (email.isNotEmpty()) scope.launch {
                    withContext(Dispatchers.IO) {
                        try { api.forgotPassword(email) } catch (_: Exception) {}
                    }
