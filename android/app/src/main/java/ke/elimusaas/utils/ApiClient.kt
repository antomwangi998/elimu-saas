...
class ApiClient(private val context: Context) {
    ...
    fun loginAsSchool(schoolCode: String): ke.elimusaas.data.LoginResponse? {
        ...
        val user = u?.let {
            ke.elimusaas.data.User(
                it.optInt("id",0), it.optString("firstName",""), it.optString("lastName",""),
    ...
    fun getPlatformStats(): org.json.JSONObject? {
        return req("GET", "/superadmin/stats") ?: req("GET", "/superadmin/schools?limit=1")?.let {
            org.json.JSONObject().put("totalSchools", it.optString("total","—"))
    ...
    fun getAllSchools(): List<SchoolItem> {
        ...
        return (0 until arr.length()).map { i -> arr.getJSONObject(i).let { o ->
            SchoolItem(o.optString("name",""), o.optString("school_code", o.optString("code","")),
                o.optString("county","Kenya"), !o.optBoolean("is_suspended",false),
    ...
    fun getSubscriptions(): List<SubscriptionItem> {
        val live = req("GET", "/superadmin/subscriptions") ?: return emptyList()
        val arr = live.optJSONArray("data") ?: live.optJSONArray("subscriptions") ?: return emptyList()
        return (0 until arr.length()).map { i -> arr.getJSONObject(i).let { o ->
            SubscriptionItem(o.optString("school_name",""), o.optString("plan","free"),
                o.optString("expiry_date",""), o.optString("status","inactive"),
                o.optString("amount","0"))
        }}
    }
