  ...
  async _renderOverview(c) {
    ...
    c.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[['Staff on Payroll',d.staffOnPayroll||0,'👔','var(--accent)'],['Total Runs (YTD)',d.total_runs||0,'📋','var(--blue)'],['Total Gross (YTD)',_money(d.total_gross_ytd),'💰','var(--green)'],['Total Net (YTD)',_money(d.total_net_ytd),'💵','var(--purple)']].map(([l,v,i,col])=>`
...
Pages.Lab = {
  ...
  async viewHistory(id,name) {
    ...
    UI.showInfoModal(`${name} -- History`,`<div style="font-size:24px;font-weight:800;text-align:center;margin-bottom:12px;color:var(--accent)">${d.quantity} ${d.unit}</div>
  ...
  async _renderReport(c) {
    ...
    const s=d.summary||{};
    c.innerHTML = `<div class="stats-grid" style="margin-bottom:20px">
      ${[['Total Items',s.total_items||0,'var(--accent)'],['Good Condition',s.good_condition||0,'var(--green)'],['Needs Attention',s.needs_attention||0,'var(--red)'],['Hazardous Items',s.hazardous_items||0,'var(--orange)'],['Total Value',_money(s.total_value),'var(--purple)']].map(([l,v,col])=>`<div class="stat-card"><div class="stat-body"><div class="stat-value" style="color:${col}">${v}</div><div class="stat-label">${l}</div></div></div>`).join('')}
    </div>
...
Pages.Alumni = {
  ...
  _grid(list) {
    ...
    return `<div class="grid-auto">${list.map(a=>`
      <div class="card">${a.is_featured?'<div style="background:var(--accent);color:#fff;font-size:9px;padding:2px 8px;border-radius:4px;margin-bottom:6px;display:inline-block">⭐ FEATURED</div>':''}
  ...
  async _renderStats(c) {
    ...
    if(d.error){c.innerHTML=UI.error(d.error);return;}
    c.innerHTML = `<div class="stats-grid" style="margin-bottom:20px">
      ${[['Total Alumni',d.total,'🎓','var(--accent)'],['Years Represented',d.years,'📅','var(--blue)'],['In University',d.in_university,'🏫','var(--green)'],['Employed',d.employed,'💼','var(--purple)'],['Avg KCSE Points',parseFloat(d.avg_kcse||0).toFixed(2),'📊','var(--cyan)']].map(([l,v,i,col])=>`<div class="stat-card"><div class="stat-body"><div class="stat-icon" style="color:${col}">${i}</div><div class="stat-value">${v}</div><div class="stat-label">${l}</div></div></div>`).join('')}
    </div>
    <div class="grid-2">
...
Pages.ParentPortal = {
  ...
  async _loadOverview(c,id) {
    ...
    c.innerHTML=`<div class="stats-grid" style="margin-bottom:16px">
      ${[['Average Marks',avgMark+'%','📊','var(--accent)'],['Attendance Rate',att.summary?.rate+'%' ||'--','✅','var(--green)'],['Fee Balance',_money(balance),'💰',balance>0?'var(--red)':'var(--green)'],['Days Present',att.summary?.present||0,'📅','var(--blue)']].map(([l,v,i,col])=>`<div class="stat-card"><div class="stat-body"><div style="font-size:20px">${i}</div><div class="stat-value" style="color:${col}">${v}</div><div class="stat-label">${l}</div></div></div>`).join('')}
    </div>
  ...
  async _loadAttendance(c,id) {
    ...
    c.innerHTML=`<div class="stats-grid" style="margin-bottom:12px">
      ${[['Total Days',summary.total||0,'var(--accent)'],['Present',summary.present||0,'var(--green)'],['Absent',summary.absent||0,'var(--red)'],['Rate',summary.rate+'%','var(--blue)']].map(([l,v,col])=>`<div class="stat-card"><div class="stat-body"><div class="stat-value" style="color:${col}">${v}</div><div class="stat-label">${l}</div></div></div>`).join('')}
    </div>
  ...
  async _loadFees(c,id) {
    ...
    c.innerHTML=`<div style="padding:16px;background:var(--${balance>0?'red':'green'}-subtle||var(--accent-subtle));border-radius:10px;text-align:center;margin-bottom:16px">
