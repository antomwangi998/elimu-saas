...
Pages.Storekeeper = {
  ...
  editItem(id, name, stock, cost, unit, reorder) {
    document.body.insertAdjacentHTML('beforeend', `
      <div class="modal-overlay open" id="edit-item-modal" onclick="if(event.target===this)this.remove()">
        <div class="modal" style="max-width:440px">
          <div class="modal-header"><h3>✏️ Edit: ${name}</h3><button onclick="document.getElementById('edit-item-modal').remove()" class="btn btn-sm">✕</button></div>
          <div class="modal-body" style="padding:20px;display:grid;grid-template-columns:1fr 1fr;gap:12px">
            <div class="form-group" style="margin:0;grid-column:1/-1"><label class="form-label">Item Name *</label>
              <input id="ei-name" class="form-control" value="${name}"></div>
            <div class="form-group" style="margin:0"><label class="form-label">Current Stock</label>
              <input id="ei-stock" class="form-control" type="number" value="${stock}" min="0"></div>
            <div class="form-group" style="margin:0"><label class="form-label">Unit Cost (KES)</label>
              <input id="ei-cost" class="form-control" type="number" value="${cost}" min="0" step="0.01"></div>
            <div class="form-group" style="margin:0"><label class="form-label">Unit</label>
              <input id="ei-unit" class="form-control" value="${unit}"></div>
            <div class="form-group" style="margin:0"><label class="form-label">Reorder Level</label>
              <input id="ei-reorder" class="form-control" type="number" value="${reorder}" min="0"></div>
          </div>
          <div class="modal-footer" style="padding:12px 20px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;gap:8px">
            <button class="btn btn-secondary" onclick="document.getElementById('edit-item-modal').remove()">Cancel</button>
            <button class="btn btn-primary" onclick="Pages.Storekeeper.saveEditItem('${id}')">💾 Save</button>
          </div>
        </div>
      </div>`);
  },
  ...
  async saveEditItem(id) {
    ...
    const r = await API.put('/storekeeper/items/'+id, {
      name, currentStock: parseFloat(document.getElementById('ei-stock')?.value)||0,
      unitCost: parseFloat(document.getElementById('ei-cost')?.value)||0,
      unit: document.getElementById('ei-unit')?.value,
      reorderLevel: parseInt(document.getElementById('ei-reorder')?.value)||0,
    });
    if (r?.id || r?.message) {
  ...
  async deleteItem(id, name) {
    ...
    const r = await API.delete('/storekeeper/items/'+id);
  ...
  async _renderReport(container) {
    ...
    container.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[['Total Items',s.totalItems||0,'var(--brand)'],['Stock Value','KES '+(parseFloat(s.totalValue||0)).toLocaleString(),'var(--green)'],['Low Stock Items',s.lowStockItems||0,'var(--amber)'],['Critical Items',s.criticalItems||0,'var(--red)']].map(([l,v,c])=>`<div class="stat-card" style="--stat-color:${c};--stat-bg:${c}1a"><div class="stat-body"><div class="stat-value" style="font-size:18px">${v}</div><div class="stat-label">${l}</div></div></div>`).join('')}
      </div>
  ...
  _openTxn(id, name, stock, unit, type='issue') {
    ...
    $id('txn-item-info').innerHTML = `<strong>${name}</strong> -- Current: <strong style="color:var(--brand)">${stock} ${unit}</strong>`;
    ...
    UI.showInfoModal(`${name} -- Transaction History`, `
      <div style="display:flex;gap:16px;margin-bottom:12px">
        <div style="background:var(--bg-elevated);padding:12px;border-radius:8px;text-align:center;flex:1">
          <div style="font-size:28px;font-weight:800;color:var(--brand)">${data.current_stock}</div>
...
Pages.Broadsheet = {
        ...
        <table style="font-size:11px;border-collapse:collapse;min-width:800px">
          ...
          <tbody>
            ${students.map((s,i)=>`<tr style="${i%2?'background:rgba(255,255,255,0.015)':''}">
              <td style="padding:6px 8px;border:1px solid var(--border-subtle);text-align:center;font-weight:700;color:var(--brand)">${s.position||i+1}</td>
              ...
              <td style="padding:6px 8px;border:1px solid var(--border-subtle);text-align:center;font-weight:700;color:var(--brand);background:rgba(43,127,255,0.04)">${parseFloat(s.mean_marks||0).toFixed(1)}</td>
          ...
          <tfoot><tr style="background:var(--bg-elevated);font-weight:700">
            ...
            ${subjects.map(sub=>{const ms=students.flatMap(s=>(s.marks||[]).filter(m=>m.subject_id===sub.id&&!m.is_absent&&m.marks!=null).map(m=>parseFloat(m.marks)));const avg=ms.length?(ms.reduce((a,b)=>a+b,0)/ms.length).toFixed(1):'--';return `<td style="padding:6px 8px;border:1px solid var(--border);text-align:center;color:var(--brand)">${avg}</td>`;}).join('')}
    ...
    c.innerHTML = `
      <div class="card" style="margin-bottom:16px">
        ...
        ${_tbl2(['Rank','Class/Stream','Students','Class Mean','Grade','Pass Rate','Highest'],
          allStats.map((s,i)=>`<tr><td style="font-weight:700;color:var(--brand)">#${i+1}</td><td><strong>${s.name}</strong></td><td>${s.count}</td><td style="font-weight:700;color:var(--${parseFloat(s.meanMarks||0)>=50?'green':'red'})">${parseFloat(s.meanMarks||0).toFixed(2)}%</td><td>${_b(s.meanGrade||'--',_gc(s.meanGrade||''))}</td><td>${parseFloat(s.passRate||0).toFixed(1)}%</td><td>${parseFloat(s.highest||0).toFixed(1)}%</td></tr>`).join(''))}
      </div>
  ...
  async _loadSubject() {
    ...
    c.innerHTML = `${_tbl2(['Rank','Subject','Avg Mark','Top Mark','Pass Rate','Students','Performance'],
      analysis.map((s,i)=>`<tr><td style="font-weight:700;color:var(--brand)">#${i+1}</td><td><strong>${s.name}</strong></td><td style="font-weight:700;color:var(--${parseFloat(s.avg)>=50?'green':'red'})">${s.avg}%</td><td>${s.top}%</td><td>${s.passRate}%</td><td>${s.attempted}</td><td style="min-width:120px"><div style="height:7px;background:var(--border);border-radius:4px;overflow:hidden"><div style="width:${s.avg}%;height:100%;background:${parseFloat(s.avg)>=70?'var(--green)':parseFloat(s.avg)>=50?'var(--amber)':'var(--red)'}"></div></div></td></tr>`).join(''))}
  ...
  async _loadTrends() {
    ...
    c.innerHTML = `<div class="card" style="margin-bottom:16px"><div class="card-header"><div class="card-title">Performance Over Time</div></div><canvas id="trd-chart" height="100"></canvas></div>
      ${_tbl2(['Exam','Avg Marks','Mean Grade','Pass Rate','Students'],
        trends.map(t=>`<tr><td><strong>${t.exam_name}</strong></td><td style="font-weight:700;color:var(--brand)">${parseFloat(t.avg_marks||0).toFixed(2)}%</td><td>${_b(t.mean_grade||'--',_gc(t.mean_grade||''))}</td><td>${t.students>0?((parseInt(t.passes||0)/parseInt(t.students))*100).toFixed(1)+'%':'--'}</td><td>${t.students||0}</td></tr>`).join(''))}`;
...
Pages.FeeClearance = {
  ...
  async generateAll() {
    Pages.FeeClearance?.load?.() || Router.go('fee-clearance');
...
Pages.ReportCards = {
  ...
  async bulkPrint() {
    ...
    Pages.ReportCards?.bulkPrint?.() || Toast.success('Opening bulk print...');
