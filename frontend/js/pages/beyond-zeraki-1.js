    ...
    ${!d.length?UI.empty('No hostels configured'):`<div class="grid-auto">${d.map(h=>`
      <div class="card" style="border-top:4px solid var(--accent)">
        ...
        <div style="display:flex;gap:10px;margin:10px 0;padding:10px;background:var(--bg-elevated);border-radius:8px">
          <div style="text-align:center;flex:1"><div style="font-size:28px;font-weight:800;color:var(--accent)">${h.current_occupancy||0}</div><div style="font-size:10px;color:var(--text-muted)">Occupied</div></div>
...
Pages.Canteen = {
  ...
  async _renderSales(c) {
    ...
    c.innerHTML=`<div class="stats-grid" style="margin-bottom:16px">
      ${[['30-day Revenue',_m(total),'var(--green)'],['Total Transactions',d.daily.reduce((s,r)=>s+parseInt(r.transactions||0),0),'var(--accent)'],['Days Active',d.daily.length,'var(--blue)']].map(([l,v,c])=>`<div class="stat-card"><div class="stat-body"><div class="stat-value" style="color:${c}">${v}</div><div class="stat-label">${l}</div></div></div>`).join('')}
    </div>
...
Pages.Health = {
  ...
  async _renderStats(c) {
    ...
    c.innerHTML=`<div class="stats-grid" style="margin-bottom:16px">
      ${[['Total Visits (30d)',d.total_visits,'var(--accent)'],['Referred',d.referred,'var(--amber)'],['Admitted',d.admitted,'var(--red)'],['In Sick Bay',d.currently_admitted,'var(--orange)']].map(([l,v,col])=>`<div class="stat-card"><div class="stat-body"><div class="stat-value" style="color:${col}">${v||0}</div><div class="stat-label">${l}</div></div></div>`).join('')}
    </div>
...
Pages.NoticBoard = {
  ...
  async render(audience) {
    ...
    `<div style="display:flex;flex-direction:column;gap:12px">${d.map(n=>`
      <div class="card" style="border-left:4px solid ${n.is_pinned?'var(--accent)':n.priority==='urgent'?'var(--red)':n.priority==='high'?'var(--amber)':'var(--border)'}">
