...
Pages.Assets = {
  ...
  async _report(c) {
    ...
    c.innerHTML=`<div class="stats-grid" style="margin-bottom:16px">${[['Total Assets',d.total||0,'var(--accent)'],['Total Cost','KES '+parseFloat(d.total_cost||0).toLocaleString(),'var(--blue)'],['Current Value','KES '+parseFloat(d.current_value||0).toLocaleString(),'var(--green)']].map(([l,v,col])=>`<div class="stat-card"><div class="stat-body"><div class="stat-value" style="color:${col}">${v}</div><div class="stat-label">${l}</div></div></div>`).join('')}</div>
...
Pages.AIEngine = {
  ...
  async doCompare() {
    ...
    if(d.error){c.innerHTML=UI.error(d.error);return;}
    c.innerHTML=`<div class="grid-3">${[['🏫 Your School',d.school,'var(--accent)'],['🏙️ County',d.county,'var(--blue)'],['🌍 National',d.national,'var(--green)']].map(([l,s,col])=>`<div class="stat-card"><div class="stat-body"><div style="font-size:32px;font-weight:800;color:${col}">${parseFloat(s?.avg||0).toFixed(1)}%</div><div class="stat-label">${l}</div><div style="font-size:11px;color:var(--text-muted)">Highest: ${parseFloat(s?.high||0).toFixed(1)}%</div></div></div>`).join('')}</div>
  ...
  async genQ() {
    ...
    <div style="display:flex;flex-direction:column;gap:12px">${(d.questions||[]).map((q,i)=>`<div class="card" style="border-left:3px solid var(--accent)"><div style="font-weight:600;margin-bottom:8px;font-size:13px">${i+1}. ${q.question}</div>
...
Pages.Wellness = {
  ...
  pick(score,btn) { this._mood=score; document.querySelectorAll('.wm-btn').forEach(b=>b.style.borderColor='var(--border)'); btn.style.borderColor='var(--accent)'; document.getElementById('mood-lbl').textContent='You selected: '+['Struggling','Not great','Okay','Good','Great!'][score-1]; },
  async submit() {
...
Pages.Polls = {
  ...
  async loadResults(pollId) {
    ...
    c.innerHTML=`<div>${(d.options||[]).map((opt,i)=>{const res=d.results?.find(r=>parseInt(r.option_index)===i);const cnt=parseInt(res?.count||0);const pct=total>0?((cnt/total)*100).toFixed(0):0;return `<div style="margin-bottom:6px"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:2px"><span>${opt.text||opt}</span><span>${pct}% (${cnt})</span></div><div style="height:8px;background:var(--border);border-radius:4px;overflow:hidden"><div style="width:${pct}%;height:100%;background:var(--accent)"></div></div></div>`;}).join('')}<div style="font-size:11px;color:var(--green);margin-top:4px">✅ You voted · ${total} total votes</div></div>`;
  },
...
Pages.Career = {
  ...
  async _resources(c) {
    ...
    `<div class="grid-auto">${d.map(r=>`<div class="card"><div class="card-header"><div class="card-title">${r.title}</div><span class="badge badge-blue">${r.resource_type}</span></div><div style="font-size:12px;color:var(--text-secondary);margin:6px 0">${(r.description||'').substring(0,100)}</div>${r.careers?.length?`<div style="font-size:11px;color:var(--accent);margin-bottom:8px">🎯 ${r.careers.join(', ')}</div>`:''}<div>${r.url?`<a href="${r.url}" target="_blank" class="btn btn-sm btn-primary">View →</a>`:''}</div></div>`).join('')}</div>`}`;
  },
  ...
  async getAdvice() {
    ...
    if(d.error){c.innerHTML=UI.error(d.error);return;}
    c.innerHTML=`<div class="card" style="border-top:4px solid var(--accent)"><div class="card-header"><div class="card-title">Career Advice</div></div><div style="font-size:13px;line-height:1.8;color:var(--text-secondary);white-space:pre-line">${d.advice}</div>${d.grades?.length?`<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">${d.grades.map(g=>`<div style="background:var(--bg-elevated);border-radius:6px;padding:6px 10px;text-align:center"><div style="font-weight:700;color:var(--accent)">${parseFloat(g.avg||0).toFixed(0)}%</div><div style="font-size:10px;color:var(--text-muted)">${g.subject}</div></div>`).join('')}</div>`:''}</div>`;
  },
  _profiles(c) {
...
Pages.Tutoring = {
  ...
  _tutCards(offers) {
    ...
    <div style="margin:6px 0;display:flex;gap:4px;flex-wrap:wrap">${(o.subjects||[]).map(s=>`<span style="background:var(--accent-subtle);color:var(--accent);padding:2px 8px;border-radius:12px;font-size:11px">${s}</span>`).join('')}</div>
    <div style="font-size:12px;color:var(--text-muted);margin-bottom:8px">${o.availability||'Contact to arrange'}</div>
    <div style="display:flex;justify-content:space-between;align-items:center">${o.is_free?'<span style="color:var(--green);font-weight:700">🆓 FREE</span>':`<span style="color:var(--accent);font-weight:700">KES ${o.rate_per_hour}/hr</span>`}
