...
// URL paste exam methods
Pages.OnlineExams.previewUrl = function(url) {
  const el = document.getElementById('exam-url-preview');
  if (!el || !url) { if(el) el.innerHTML=''; return; }
  try {
    const u = new URL(url);
    const icon = u.hostname.includes('google')?'🔵 Google Forms':u.hostname.includes('elimu')?'📋 ElimuSaaS':'🔗 External Exam';
    el.innerHTML = `<div style="background:var(--bg-elevated);border:1px solid var(--border);border-radius:8px;padding:8px 12px;font-size:12px;display:flex;align-items:center;gap:8px"><span>${icon}</span><span style="color:var(--text-muted)">${u.hostname}</span><span class="badge badge-green" style="margin-left:auto">Valid ✓</span></div>`;
  } catch { el.innerHTML = '<div style="color:var(--red);font-size:12px">⚠️ Invalid URL</div>'; }
};
Pages.OnlineExams.openUrl = function() {
  const url = document.getElementById('exam-url-input')?.value?.trim();
  if (!url) { Toast.error('Paste an exam URL first'); return; }
  try { new URL(url); } catch { Toast.error('Invalid URL'); return; }
  window.open(url, '_blank', 'noopener,noreferrer');
  Toast.info('Exam opened in a new tab. Return here when done to record your submission.');
  API.post('/online-exams/student-submit', { examUrl: url, submittedAt: new Date().toISOString() }).catch(()=>{});
};
