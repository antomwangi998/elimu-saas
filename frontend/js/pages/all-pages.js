    ...
    grid.innerHTML = data.map(c => `
      <div class="card" style="cursor:pointer" onclick="Pages.Clubs.viewClub('${c.id}')">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
          <div style="width:48px;height:48px;border-radius:var(--radius);background:var(--accent-subtle);display:flex;align-items:center;justify-content:center;font-size:24px">
...
Pages.Communication = {
  ...
  async renderNotifications(container) {
    ...
    container.innerHTML = `<div style="display:flex;justify-content:flex-end;margin-bottom:8px"><button class="btn btn-sm btn-ghost" onclick="Pages.Communication.markAllRead()">Mark all read</button></div>` +
      data.map(n => `
        <div style="padding:10px 14px;background:${n.is_read ? 'var(--bg-elevated)' : 'var(--accent-subtle)'};border-radius:var(--radius);border:1px solid var(--border);margin-bottom:6px">
...
Pages.SuperAdmin = {
  async load() {
    ...
    if (statsEl) statsEl.innerHTML = [
      { value: data.schools?.active || 0, label: 'Active Schools', color: 'var(--green)', bg: 'var(--green-bg)', icon: '🏫' },
      { value: parseInt(data.students?.total || 0).toLocaleString(), label: 'Total Students', color: 'var(--accent)', bg: 'var(--accent-subtle)', icon: '👨‍🎓' },
      { value: UI.currency(data.revenue?.total || 0), label: 'Revenue (This Year)', color: 'var(--amber)', bg: 'var(--amber-bg)', icon: '💰' },
      { value: data.revenue?.transactions || 0, label: 'Transactions', color: 'var(--purple)', bg: 'var(--purple-bg)', icon: '📊' },
    ].map(s => `
    ...
    if (recent) recent.innerHTML = (data.recentSchools || []).map(s => `
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border-subtle)">
        <div><div style="font-weight:600;font-size:13px">${s.name}</div><div style="font-size:11px;color:var(--text-muted)">${s.school_code} · ${UI.date(s.created_at)}</div></div>
        <span class="badge ${s.sub_status === 'active' ? 'badge-green' : s.sub_status === 'trial' ? 'badge-amber' : 'badge-gray'}">${s.sub_status || 'No sub'}</span>
      </div>`).join('');
  ...
  async loadSchools() {
    ...
    if (data.error) { tbody.innerHTML = `<tr><td colspan="7" style="text-align:center">${data.error}</td></tr>`; return; }
    ...
    tbody.innerHTML = list.map(s => `
      <tr>
        ...
        <td><span class="badge ${s.subscription_status === 'active' ? 'badge-green' : s.subscription_status === 'trial' ? 'badge-amber' : 'badge-gray'}">${s.subscription_status || 'No subscription'}</span></td>
        <td><span class="badge ${s.is_active ? 'badge-green' : 'badge-red'}">${s.is_active ? 'Active' : 'Inactive'}</span></td>
        <td><button class="btn btn-sm btn-ghost" onclick="Toast.info('Managing ${s.name}')">Manage</button></td>
      </tr>`).join('');
  },
