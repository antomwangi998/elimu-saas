  ...
  async _renderOverview(c) {
    ...
    c.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[
          ['Total Schools', d.total_schools, '🏫', 'var(--brand)'],
  ...
  _schoolsTable(schools) {
    ...
    return `<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12px">
      <thead><tr style="background:var(--bg-elevated)">
        ${['School','Code','Active','Sub Status','Plan','Students','Staff','Expires','Reset PWD','Actions'].map(h =>
      ...
      <tbody>
        ${schools.map(s => `<tr style="border-bottom:1px solid var(--border-subtle)">
          ...
          <td style="padding:8px 10px;font-family:monospace;color:var(--brand)">${s.code}</td>
          <td style="padding:8px 10px;text-align:center"><span class="badge badge-${s.is_active?'green':'red'}">${s.is_active?'✅ Active':'🔒 Suspended'}</span></td>
          <td style="padding:8px 10px"><span class="badge badge-${stC[s.subscription_status]||'gray'}">${s.subscription_status||'--'}</span>${!s.is_active?'<div style="font-size:10px;color:var(--red);margin-top:2px">⚠️ LOCKED OUT</div>':''}</td>
          ...
          <td style="padding:8px 10px;font-size:11px">${s.subscription_expires_at ? new Date(s.subscription_expires_at).toLocaleDateString('en-KE') : '--'}</td>
          <td style="padding:8px 10px;text-align:center"><button class="btn btn-sm btn-ghost" style="color:var(--amber)" onclick="Pages.SuperAdmin.resetSchoolAdminPwd('${s.id}','${s.name.replace(/'/g,"\\'")}')" >🔑 Reset</button></td>
          <td style="padding:8px 10px">
...
// ============================================================
Pages.SuperAdminAnalytics = {
  async load() {
    const area = document.getElementById('sa-analytics-content');
    if (!area) return;
    area.innerHTML = `<div style="text-align:center;padding:60px"><div class="loading-spinner" style="margin:auto"></div><p style="color:var(--text-muted);margin-top:16px">Loading platform metrics...</p></div>`;

    const [stats, schools] = await Promise.all([
      API.get('/superadmin/stats').catch(()=>({})),
      API.get('/superadmin/schools?limit=200').catch(()=>[]),
    ]);

    const schoolList = Array.isArray(schools) ? schools : (schools.schools || schools.data || []);
    ...
    const grace     = schoolList.filter(s => s.subscription_status === 'grace').length;
    const suspended = schoolList.filter(s => s.subscription_status === 'suspended').length;
    const total     = schoolList.length;

    // Estimated MRR: assume basic plan KES 3,000/school/mo
    const mrr = active * 3000;
    const arr = mrr * 12;
    const churnRate = total > 0 ? ((suspended / total) * 100).toFixed(1) : '0.0';

    // Recent signups (last 30 days)
    const thirtyDaysAgo = Date.now() - 30 * 24 * 60 * 60 * 1000;
    const recentSignups = schoolList.filter(s => s.created_at && new Date(s.created_at).getTime() > thirtyDaysAgo).length;
    ...
    area.innerHTML = `
      ...
      <div class="stats-grid" style="margin-bottom:24px">
        <div class="stat-card" style="--stat-color:var(--brand);--stat-bg:var(--brand-subtle)">
          ...
          <div class="stat-body"><div class="stat-value">${total}</div><div class="stat-label">Total Schools</div></div>
        </div>
        <div class="stat-card" style="--stat-color:var(--green);--stat-bg:var(--green-bg)">
          <div class="stat-icon">✅</div>
          <div class="stat-body"><div class="stat-value">${active}</div><div class="stat-label">Active Subscriptions</div></div>
        </div>
        <div class="stat-card" style="--stat-color:var(--amber);--stat-bg:var(--amber-bg)">
          <div class="stat-icon">💰</div>
          <div class="stat-body"><div class="stat-value">KES ${(mrr/1000).toFixed(0)}K</div><div class="stat-label">Est. MRR</div></div>
        </div>
        <div class="stat-card" style="--stat-color:var(--purple);--stat-bg:var(--purple-bg)">
          <div class="stat-icon">📈</div>
          <div class="stat-body"><div class="stat-value">KES ${(arr/1000).toFixed(0)}K</div><div class="stat-label">Est. ARR</div></div>
        </div>
      </div>
      ...
      <div class="stats-grid" style="margin-bottom:28px">
        <div class="stat-card" style="--stat-color:var(--cyan);--stat-bg:var(--cyan-bg)">
          <div class="stat-icon">🆕</div>
          <div class="stat-body"><div class="stat-value">${recentSignups}</div><div class="stat-label">New Schools (30d)</div></div>
        </div>
        <div class="stat-card" style="--stat-color:var(--amber);--stat-bg:var(--amber-bg)">
          <div class="stat-icon">⏳</div>
          <div class="stat-body"><div class="stat-value">${trial}</div><div class="stat-label">On Trial</div></div>
        </div>
        <div class="stat-card" style="--stat-color:var(--orange);--stat-bg:var(--orange-bg)">
          <div class="stat-icon">⚠️</div>
          <div class="stat-body"><div class="stat-value">${grace}</div><div class="stat-label">Grace Period</div></div>
        </div>
        <div class="stat-card" style="--stat-color:var(--red);--stat-bg:var(--red-bg)">
          <div class="stat-icon">📉</div>
          <div class="stat-body"><div class="stat-value">${churnRate}%</div><div class="stat-label">Churn Rate</div></div>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:20px">
        <div class="card">
          <div class="card-header"><h3>📊 Subscription Breakdown</h3></div>
          <div style="padding:20px">
            ${[
              { label: 'Active', count: active, color: 'var(--green)', pct: total ? (active/total*100).toFixed(0) : 0 },
              { label: 'Trial',  count: trial,  color: 'var(--amber)', pct: total ? (trial/total*100).toFixed(0) : 0 },
              { label: 'Grace',  count: grace,  color: 'var(--orange)', pct: total ? (grace/total*100).toFixed(0) : 0 },
              { label: 'Suspended', count: suspended, color: 'var(--red)', pct: total ? (suspended/total*100).toFixed(0) : 0 },
            ].map(row => `
              <div style="margin-bottom:14px">
                <div style="display:flex;justify-content:space-between;margin-bottom:5px;font-size:13px">
                  <span style="color:${row.color};font-weight:600">${row.label}</span>
                  <span>${row.count} schools (${row.pct}%)</span>
                </div>
                <div style="height:8px;background:var(--border);border-radius:99px;overflow:hidden">
                  <div style="height:100%;width:${row.pct}%;background:${row.color};border-radius:99px;transition:width 0.6s ease"></div>
                </div>
              </div>
            `).join('')}
          </div>
        ...
        <div class="card">
          <div class="card-header"><h3>💡 Revenue Insights</h3></div>
          <div style="padding:20px">
            <div style="display:flex;flex-direction:column;gap:14px">
              <div style="background:var(--brand-subtle);border-radius:12px;padding:16px">
                <div style="font-size:12px;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">Monthly Recurring Revenue</div>
                <div style="font-size:28px;font-weight:800;color:var(--brand)">KES ${mrr.toLocaleString()}</div>
                <div style="font-size:12px;color:var(--text-muted);margin-top:2px">Based on ${active} active schools @ KES 3,000/mo</div>
              </div>
              <div style="background:var(--green-bg);border-radius:12px;padding:16px">
                <div style="font-size:12px;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">Annual Run Rate</div>
                <div style="font-size:24px;font-weight:800;color:var(--green)">KES ${arr.toLocaleString()}</div>
              </div>
              <div style="background:var(--bg-elevated);border-radius:12px;padding:14px;font-size:13px;color:var(--text-muted)">
                💡 <strong>Growth opportunity:</strong> ${trial} trial schools are potential revenue. Converting them all would add <strong>KES ${(trial*3000).toLocaleString()}/mo</strong>.
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="card">
        <div class="card-header">
          <h3>🕐 Recently Joined Schools</h3>
          <button class="btn btn-sm btn-secondary" onclick="Router.go('superadmin-schools')">View All →</button>
        </div>
        <div style="overflow-x:auto">
          <table>
            <thead><tr><th>School</th><th>Code</th><th>Status</th><th>Joined</th><th>Action</th></tr></thead>
            <tbody>
              ${schoolList.slice().sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0)).slice(0,10).map(s=>`
                <tr>
                  <td><strong>${s.name}</strong></td>
                  <td class="font-mono text-sm">${s.code||'—'}</td>
                  <td><span class="badge badge-${s.subscription_status==='active'?'green':s.subscription_status==='trial'?'amber':s.subscription_status==='grace'?'orange':'red'}">${s.subscription_status||'—'}</span></td>
                  <td style="color:var(--text-muted);font-size:12px">${s.created_at ? new Date(s.created_at).toLocaleDateString('en-KE',{day:'numeric',month:'short',year:'numeric'}) : '—'}</td>
                  <td><button class="btn btn-sm btn-ghost" onclick="Pages.SuperAdmin.manageSchool('${s.id}','${(s.name||'').replace(/'/g,"\\'")}','${s.subscription_status}')">Manage</button></td>
                </tr>`).join('')}
            </tbody>
          </table>
        </div>
...
Router.define?.('superadmin-analytics', { title: 'Platform Analytics', onEnter: () => Pages.SuperAdminAnalytics.load() });
