...
var _err     = (msg='Error') => `<div class="alert alert-danger"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><circle cx="12" cy="12" r="10"/><path d="M12 8v4m0 4h.01"/></svg><span>${msg}</span></div>`;
      ...
      <div class="card" style="cursor:pointer" onclick="Pages.Exams.openPaperEntry('${p.id}','${p.subject_name}')">
        ...
        <div style="margin-top:10px">
          <div style="height:4px;background:var(--border);border-radius:2px;overflow:hidden">
            <div style="width:${p.student_count>0?(p.marks_entered/p.student_count*100).toFixed(0):0}%;height:100%;background:var(--accent)"></div>
          </div>
        </div>
      </div>`).join('')}</div>`;
  },
    ...
    c.innerHTML = `
      <div style="overflow-x:auto">
        <table>
          ...
          <tbody>
            ${students.map((s,i)=>`<tr>
              <td style="font-weight:700;color:var(--accent)">${s.position||i+1}</td>
              ...
              <td style="font-weight:700;color:var(--accent)">${s.mean_marks==='--'?'--':parseFloat(s.mean_marks||0).toFixed(1)}</td>
      ...
      <div style="display:flex;gap:16px;flex-wrap:wrap;margin-top:12px;padding:12px;background:var(--bg-elevated);border-radius:var(--radius)">
        ${[['Students',stats.total||students.length],['Class Mean',parseFloat(stats.meanMarks||0).toFixed(1)+'%'],['Pass Rate',parseFloat(stats.passRate||0).toFixed(1)+'%'],['Top Score',parseFloat(stats.highest||0).toFixed(1)+'%']].map(([l,v])=>`<div style="text-align:center"><div style="font-size:20px;font-weight:800;color:var(--accent)">${v}</div><div style="font-size:11px;color:var(--text-muted)">${l}</div></div>`).join('')}
      </div>`;
  },
  ...
  async renderSummary(container){
    ...
    container.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[['Present Today',s.present||0,'var(--green)','var(--green-bg)'],['Absent',s.absent||0,'var(--red)','var(--red-bg)'],['Late',s.late||0,'var(--amber)','var(--amber-bg)'],['Rate',rate+'%','var(--accent)','var(--accent-subtle)']].map(([l,v,c,bg])=>`
...
Pages.Communication = {
  ...
  async renderNotifications(container){
    ...
    container.innerHTML = list.map(n=>`
      <div style="padding:10px 12px;background:${n.is_read?'var(--bg-elevated)':'var(--accent-subtle)'};border-radius:var(--radius);margin-bottom:6px;border:1px solid ${n.is_read?'var(--border)':'rgba(43,127,255,0.3)'};cursor:pointer" onclick="Pages.Communication.markRead('${n.id}')">
...
Pages.Reports = {
  ...
  async renderAcademic(container){
    ...
    container.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[['Total Students',stats.total||0,'var(--accent)'],['Boys',stats.boys||0,'var(--blue)'],['Girls',stats.girls||0,'var(--pink)'],['Boarding',stats.boarding||0,'var(--purple)']].map(([l,v,c])=>`
      ...
      <div class="card">
        ...
        <div style="overflow-x:auto">${_tbl(['Class','Level','Students','Boys','Girls','Boarding'],
          byClass.map(c=>`<tr>
            ...
            <td style="color:var(--accent)">${c.boys||0}</td>
  ...
  async renderFinance(container){
    ...
    container.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[['Total Collected',UI.currency(s.total_collected||0),'var(--green)'],['Transactions',parseInt(s.transaction_count||0).toLocaleString(),'var(--accent)'],['Students Paid',parseInt(s.students_who_paid||0).toLocaleString(),'var(--cyan)'],['Avg Payment',UI.currency(s.avg_payment||0),'var(--purple)']].map(([l,v,c])=>`
          <div class="stat-card" style="--stat-color:${c};--stat-bg:${c}1a">
      ...
      <div class="grid-2">
        <div class="card">
          ...
          ${[['M-Pesa',s.mpesa_amount,'var(--green)'],['Cash',s.cash_amount,'var(--accent)'],['Bank',s.bank_amount,'var(--purple)']].map(([l,v,c])=>{
  ...
  async renderAttendance(container){
    ...
    container.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[['Overall Rate',rate+'%','var(--green)'],['Present',parseInt(overall.present||0).toLocaleString(),'var(--accent)'],['Absent',parseInt(overall.absent||0).toLocaleString(),'var(--red)'],['Days Recorded',data.daysRecorded||0,'var(--purple)']].map(([l,v,c])=>`
          <div class="stat-card" style="--stat-color:${c};--stat-bg:${c}1a">
            <div class="stat-body"><div class="stat-value">${v}</div><div class="stat-label">${l}</div></div>
          </div>`).join('')}
      </div>
  ...
  async view(id){
    ...
    UI.showInfoModal(`${data.first_name} ${data.last_name}`, `
      <div class="grid-2">
        ...
        <div>
          ${data.showcase_quote?`<div style="background:var(--accent-subtle);border-left:3px solid var(--accent);padding:12px;border-radius:var(--radius);margin-bottom:12px;font-style:italic">"${data.showcase_quote}"</div>`:''}
      ...
      <div class="grid-2">
        ...
        <div class="card">
          ...
          ${(terms||[]).map(t=>`<div style="padding:8px 0;border-bottom:1px solid var(--border-subtle);font-size:13px">
            <strong>${t.term?.replace('_',' ')}</strong> ${y.year||''} ${t.is_current?_badge('Current','green'):''}</div>`).join('')||'<div style="padding:16px;color:var(--text-muted)">No terms configured</div>'}
        </div>
      </div>`;
  },
      ...
      <div class="card">
        ...
        ${_tbl(['Grade','Min Marks','Max Marks','Points','Description'],
          list.map(g=>`<tr>
            ...
            <td>${g.min_marks}</td>
            <td>${g.max_marks}</td>
            <td style="font-weight:700;color:var(--accent)">${g.points}</td>
...
Pages.SuperAdmin = {
  async load(){
    ...
    if(saStats){
      saStats.innerHTML = [
        {l:'Total Schools',v:s.totalSchools||0,c:'var(--accent)',bg:'var(--accent-subtle)',i:'🏫'},
        ...
        {l:'Monthly Revenue',v:'KES '+parseInt(s.monthlyRevenue||0).toLocaleString(),c:'var(--purple)',bg:'var(--purple-bg)',i:'💰'},
  ...
  async loadSchools(){
    ...
    tbody.innerHTML = list.map(s=>`<tr>
      ...
      <td><div style="display:flex;gap:4px">
        ...
        ${s.is_active?`<button class="btn btn-sm btn-ghost" style="color:var(--amber)" onclick="Pages.SuperAdmin.suspend('${s.id}','${s.name}')">Suspend</button>`:`<button class="btn btn-sm btn-ghost" onclick="Pages.SuperAdmin.activate('${s.id}')">Activate</button>`}
      </div></td>
    </tr>`).join('');
  ...
  async loadAnalytics() {
    ...
    el.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[['Total Revenue','KES '+(parseInt(s.totalRevenue||0)).toLocaleString(),'💰','var(--green)'],['Monthly Revenue','KES '+(parseInt(s.monthlyRevenue||0)).toLocaleString(),'📈','var(--accent)'],['Total Schools',s.totalSchools||0,'🏫','var(--cyan)'],['Total Students',(parseInt(s.totalStudents||0)).toLocaleString(),'👥','var(--purple)']].map(([l,v,i,c])=>`
          <div class="stat-card" style="--stat-color:${c};--stat-bg:${c}1a">
    ...
    container.innerHTML = `
      ...
      ${classes.map((cls,ci)=>`
        <div id="tt-cls-${cls.id}" style="display:${ci===0?'block':'none'}">
          <div style="font-weight:700;font-size:15px;color:var(--accent);margin-bottom:10px">${cls.name} -- Timetable</div>
          <div style="overflow-x:auto">
      ...
      <div class="card">
        ...
        ${!byDay[day].length ? '<div style="text-align:center;padding:20px;color:var(--text-muted);font-size:12px">Free</div>' :
          byDay[day].sort((a,b)=>a.sort_order-b.sort_order).map(s=>`
            <div style="padding:8px;margin-bottom:6px;background:var(--bg-elevated);border-radius:6px;border-left:3px solid var(--accent)">
...
Pages.Billing = {
  ...
  async renderSummary(container){
    ...
    container.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[['Total Invoices',s.total_invoices||0,'var(--accent)','var(--accent-subtle)'],['Paid',s.paid_count||0,'var(--green)','var(--green-bg)'],['Unpaid',s.unpaid_count||0,'var(--red)','var(--red-bg)'],['Collection',rate+'%','var(--amber)','var(--amber-bg)']].map(([l,v,c,bg])=>`
          <div class="stat-card" style="--stat-color:${c};--stat-bg:${bg}">
            <div class="stat-body"><div class="stat-value">${v}</div><div class="stat-label">${l}</div></div>
          </div>`).join('')}
      </div>
      <div class="grid-2">
        <div class="card">
          ...
          ${[['Total Billed','KES '+parseFloat(s.total_billed||0).toLocaleString(),'var(--accent)'],['Total Paid','KES '+parseFloat(s.total_paid||0).toLocaleString(),'var(--green)'],['Outstanding','KES '+parseFloat(s.total_outstanding||0).toLocaleString(),'var(--red)']].map(([l,v,c])=>`
...
Pages.TSCVerif = {
  ...
  async viewFraudFlags(){
    ...
    const sevBg={critical:'var(--red-bg)',high:'var(--amber-bg)',medium:'var(--accent-subtle)'};
...
Pages.AIInsights = {
  ...
  async renderInsights(container){
    ...
    const sevBg={critical:'var(--red-bg)',warning:'var(--amber-bg)',info:'var(--accent-subtle)',success:'var(--green-bg)'};
    const sevBorder={critical:'var(--red)',warning:'var(--amber)',info:'var(--accent)',success:'var(--green)'};
    container.innerHTML = `
      <div style="display:flex;justify-content:flex-end;margin-bottom:12px">
  ...
  async loadTrends(classId){
    ...
    c.innerHTML = `
      ...
      ${_tbl(['Exam','Avg Marks','Students','Pass Rate'],
        trends.map(t=>`<tr>
          ...
          <td style="font-weight:700;color:var(--accent)">${t.avg_marks||'--'}%</td>
          <td>${t.students||0}</td>
    ...
    if(window.Chart && trends.length>1){
      new Chart(document.getElementById('ai-chart'),{
        type:'line',
        data:{labels:trends.map(t=>t.exam_name?.substring(0,20)),datasets:[{label:'Avg Marks %',data:trends.map(t=>parseFloat(t.avg_marks||0)),borderColor:'var(--accent)',backgroundColor:'rgba(43,127,255,0.1)',tension:0.4,fill:true}]},
    ...
    container.innerHTML = `
      ...
      ${_tbl(['Rank','Student','Class','Points','Academic','Attendance','Badges'],
        (data||[]).map((s,i)=>`<tr>
          ...
          <td>${s.class_name||'--'}</td>
          <td style="font-size:18px;font-weight:800;color:var(--accent)">${s.total_points||0}</td>
    ...
    el.innerHTML = this.threads.map(t=>`
      <div onclick="Pages.Threads.open('${t.id}')" style="padding:12px 14px;border-bottom:1px solid var(--border);cursor:pointer;background:${this.currentId===t.id?'var(--bg-elevated)':''};" onmouseenter="this.style.background='var(--bg-elevated)'" onmouseleave="this.style.background='${this.currentId===t.id?'var(--bg-elevated)':''}'">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          ...
          ${parseInt(t.unread_count||0)>0?`<span style="background:var(--accent);color:#fff;border-radius:999px;padding:2px 6px;font-size:10px;font-weight:700">${t.unread_count}</span>`:''}
        </div>
  ...
  async loadMsgs(threadId){
    ...
    c.innerHTML = msgs.map(m=>{
      ...
      return `<div style="display:flex;justify-content:${mine?'flex-end':'flex-start'}">
        <div style="max-width:72%;background:${mine?'var(--accent)':'var(--bg-elevated)'};color:${mine?'#fff':'var(--text-primary)'};padding:10px 14px;border-radius:${mine?'18px 18px 4px 18px':'18px 18px 18px 4px'};">
...
Pages.ParentPortal = {
  ...
  async load(){
    ...
    grid.innerHTML = data.map(c=>`
      <div class="card" style="cursor:pointer;text-align:center;transition:transform 0.2s" onclick="Pages.ParentPortal.select('${c.id}')" onmouseenter="this.style.transform='translateY(-2px)'" onmouseleave="this.style.transform=''">
        ${c.photo_url?`<img src="${c.photo_url}" style="width:72px;height:72px;border-radius:50%;object-fit:cover;margin:0 auto 10px;border:3px solid var(--accent)">`:`<div class="avatar xl" style="margin:0 auto 10px">${c.first_name[0]}</div>`}
  ...
  async childDashboard(container){
    ...
    container.innerHTML = `
      <div class="stats-grid" style="margin-bottom:20px">
        ${[['Attendance',parseFloat(attendance.rate||0).toFixed(1)+'%','var(--green)'],['Fee Balance','KES '+parseFloat(fees.balance||0).toLocaleString(),'var(--red)'],['Avg Marks',latestExam.avg_marks?parseFloat(latestExam.avg_marks).toFixed(1)+'%':'--','var(--accent)'],['Absences',attendance.absent||0,'var(--amber)']].map(([l,v,c])=>`
          <div class="stat-card" style="--stat-color:${c};--stat-bg:${c}1a">
            <div class="stat-body"><div class="stat-value">${v}</div><div class="stat-label">${l}</div></div>
          </div>`).join('')}
      </div>
  ...
  async childAttendance(container){
    ...
    container.innerHTML = `
      <div class="stats-grid" style="margin-bottom:16px">
        ${[['Present',s.present||0,'var(--green)'],['Absent',s.absent||0,'var(--red)'],['Late',s.late||0,'var(--amber)'],['Rate',parseFloat(s.rate||0).toFixed(1)+'%','var(--accent)']].map(([l,v,c])=>`
  ...
  async renderShowcase(container){
    ...
    container.innerHTML = !data.length ? `${_empty('No alumni featured','Feature outstanding alumni to inspire current students.')}<div style="text-align:center"><button class="btn btn-secondary" onclick="Router.go('alumni')">Go to Alumni</button></div>` : `
      <div class="grid-auto">${data.map(a=>`
        <div class="card" style="text-align:center">
          ${a.showcase_image_url?`<img src="${a.showcase_image_url}" style="width:72px;height:72px;border-radius:50%;object-fit:cover;margin:0 auto 10px;border:3px solid var(--accent)">`:`<div class="avatar xl" style="margin:0 auto 10px">${a.first_name[0]}</div>`}
          ...
          ${a.current_occupation?`<div style="font-size:12px;color:var(--accent);margin-top:4px">${a.current_occupation}</div>`:''}
