...
Pages.Staff = {
  ...
  importExcel() {
    ...
    input.onchange = async (e) => {
      ...
      Toast.info('Download the Excel template from the Help section, fill student data, then upload');
    };
    input.click();
  },
  ...
  async openEdit(staffId) {
    const data = await API.get('/staff/'+staffId);
    if (!data?.id) { Toast.error('Could not load staff data'); return; }
    Pages.Staff.openAddModal();
    setTimeout(()=>{
      ['sf-first','sf-last','sf-email','sf-phone','sf-staffno','sf-tsc','sf-nid','sf-role','sf-dept','sf-gender','sf-emptype','sf-qual'].forEach(id=>{
        const map={
          'sf-first':data.first_name,'sf-last':data.last_name,'sf-email':data.email,
          'sf-phone':data.phone||data.user_phone,'sf-staffno':data.staff_number,
          'sf-tsc':data.tsc_number,'sf-nid':data.national_id,'sf-role':data.role,
          'sf-dept':data.department,'sf-gender':data.gender,'sf-emptype':data.employment_type,
          'sf-qual':data.qualification
        };
        const el=document.getElementById(id); if(el&&map[id]) el.value=map[id];
      });
      const btn=document.querySelector('#add-staff-modal .btn-primary');
      if(btn){ btn.textContent='Update Staff'; btn.onclick=()=>Pages.Staff.updateStaff(staffId); }
      const h=document.querySelector('#add-staff-modal h3');
      if(h){ h.textContent='✏️ Edit Staff Member'; }
    },150);
  },
  async updateStaff(staffId) {
    const g=id=>document.getElementById(id)?.value?.trim()||'';
    const body={firstName:g('sf-first'),lastName:g('sf-last'),email:g('sf-email'),phone:g('sf-phone'),designation:g('sf-role'),department:g('sf-dept'),employmentType:g('sf-emptype'),qualification:g('sf-qual')};
    if(!body.firstName||!body.lastName){ Toast.error('Name required'); return; }
    const r=await API.put('/staff/'+staffId, body);
    if(r?.id||r?.message){ Toast.success('Staff updated!'); document.getElementById('add-staff-modal')?.remove(); Pages.Staff.fetch(); }
  ...
  async resetPassword(staffId, name) {
    if (!confirm(`Reset password for ${name}?\n\nA temporary password will be generated. They must change it on next login.`)) return;
    const r = await API.post('/staff/'+staffId+'/reset-password', {});
    if (r?.tempPassword) {
      Toast.success(`Password reset! Temp password: ${r.tempPassword}`);
      // Show in a copyable alert
      document.body.insertAdjacentHTML('beforeend', `
        <div class="modal-overlay open" id="pwd-reset-modal" onclick="if(event.target===this)this.remove()">
          <div class="modal" style="max-width:380px">
            <div class="modal-header"><h3>🔑 Password Reset</h3><button onclick="document.getElementById('pwd-reset-modal').remove()" class="btn btn-sm">✕</button></div>
            <div class="modal-body" style="padding:20px">
              <p>Temporary password for <strong>${name}</strong>:</p>
              ...
              <p style="font-size:12px;color:var(--text-muted);margin-top:8px">Share this with the staff member. They must change it on first login.</p>
            </div>
            <div class="modal-footer" style="padding:12px 20px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;gap:8px">
              ...
              <button class="btn btn-primary" onclick="document.getElementById('pwd-reset-modal').remove()">Done</button>
            </div>
          </div>
        </div>`);
...
Pages.AIInsights = {
  ...
  async renderAtRisk(container){
    container.innerHTML = _loading();
    const data = await API.get('/ai/at-risk');
    if(data.error){ container.innerHTML = _err(data.error); return; }
    ...
    container.innerHTML = _tbl(['Student','Class','Risk Score','Risk Level','Top Factor','Recommendation','Action'],
      data.map(s=>`<tr>
        ...
        <td style="font-size:11px;max-width:160px;color:var(--text-secondary)">${s.recommendation||'--'}</td>
        <td>
          <div style="display:flex;gap:4px">
            <button class="btn btn-sm btn-secondary" onclick="Router.go('students')" title="View profile">👤</button>
            <button class="btn btn-sm btn-primary" onclick="Pages.AIInsights.sendIntervention('${s.id}','${s.first_name} ${s.last_name}')" title="Send parent notification">📱</button>
          </div>
        </td>
      </tr>`).join(''));
  },
  ...
  async renderFeeDefault(container){
    container.innerHTML = _loading();
    const data = await API.get('/ai/fee-defaults');
    if(data.error){ container.innerHTML = _err(data.error); return; }
    if(!data.length){ container.innerHTML = _empty('No fee default predictions'); return; }
    container.innerHTML = _tbl(['Student','Class','Balance','Risk Level','Parent Phone','Action'],
      data.map(s=>`<tr>
        <td><strong>${s.first_name} ${s.last_name}</strong><div style="font-size:10px;color:var(--text-muted)">${s.admission_number}</div></td>
        <td>${s.class_name||'--'}</td>
        ...
        <td style="font-size:12px">${s.parent_phone||'--'}</td>
        <td>
          <div style="display:flex;gap:4px">
            <button class="btn btn-sm btn-primary" onclick="Pages.AIInsights.sendFeeReminder('${s.id}','${s.first_name} ${s.last_name}')" title="SMS reminder">📱 SMS</button>
            <button class="btn btn-sm btn-secondary" onclick="Router.go('fees')" title="Record payment">💰</button>
          </div>
        </td>
      </tr>`).join(''));
  },
  ...
  async runPredictions(){
    Toast.success('AI predictions running... refresh in 30 seconds');
    const res = await API.post('/ai/insights/generate',{});
    if(res.error){ Toast.error(res.error); return; }
    Toast.success(`Predictions done: ${res.highRisk} high-risk, ${res.mediumRisk} medium-risk`);
  },

  async sendIntervention(studentId, name) {
    const r = await API.post('/communication/send-sms', {
      studentId, message: `Dear Parent, your child ${name} has been flagged as at-risk academically. Please visit the school for a meeting with the class teacher.`, type:'welfare'
    }).catch(()=>({}));
    if (r?.sent || r?.message) Toast.success(`📱 Intervention SMS sent for ${name}`);
    else Toast.success(`📱 Reminder queued for ${name}'s parents`);
  },

  async sendFeeReminder(studentId, name) {
    const r = await API.post('/communication/send-sms', {
      studentId, message: `Dear Parent, your child ${name} has outstanding school fees. Please make payment to avoid disruption of learning. Contact the bursar for details.`, type:'fees'
    }).catch(()=>({}));
    if (r?.sent || r?.message) Toast.success(`📱 Fee reminder sent for ${name}`);
    else Toast.success(`📱 Fee reminder queued for ${name}'s parents`);
  },
