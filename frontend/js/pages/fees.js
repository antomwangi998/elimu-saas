    ...
    container.innerHTML = `
      <div class="table-container">
        <div class="table-header">
          ...
          <button class="btn btn-sm btn-secondary" id="sms-blast-btn" onclick="Pages.Fees.smsBlastDefaulters(${defaulters.length})">
            ...
            SMS All ${defaulters.length} Parents
          </button>
        </div>
        <div style="overflow-x:auto">
          <table>
  ...
  printInvoice(id, name) {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>Invoice — ${name}</title>
      ...
      <div style="background:#F9F9F9;padding:14px;border-radius:8px;font-size:12px;color:#555;margin-top:16px">
        <strong>Payment Details:</strong> Paybill: 400200 | Account: ${school.school_code||'SCHOOL'}-${name.split(' ')[0].toUpperCase()}
      </div>
...
// ── Fee Defaulter SMS Blast ──────────────────────────────────
Pages.Fees.smsBlastDefaulters = async function(count) {
  if (!count || count === 0) { Toast.info('No defaulters to notify.'); return; }

  const confirmed = confirm(
    `📱 SMS Blast — Fee Defaulters\n\n` +
    `This will send an SMS to all ${count} parents with outstanding fees.\n\n` +
    `Message will include:\n` +
    `• Student name & admission number\n` +
    `• Amount outstanding\n` +
    `• School name & payment instructions\n\n` +
    `Proceed?`
  );
  if (!confirmed) return;

  const btn = document.getElementById('sms-blast-btn');
  if (btn) { btn.disabled = true; btn.textContent = 'Sending...'; }

  try {
    const res = await API.post('/fees/sms-blast-defaulters', {});
    if (res.error) {
      Toast.error('SMS blast failed: ' + res.error);
    } else {
      const sent = res.sent || count;
      const failed = res.failed || 0;
      Toast.success(
        `✅ SMS blast complete! ${sent} messages sent${failed > 0 ? `, ${failed} failed` : ''}.`,
        'SMS Sent'
      );
    }
  } catch (e) {
    Toast.error('Could not send SMS blast. Check your internet connection.');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = `SMS All ${count} Parents`; }
  }
};
