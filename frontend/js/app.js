...
window.CONFIG  = window.CONFIG  || {API_URL:'/api'};
...
function buildSidebar() {
  ...
  if (userAvatarEl) userAvatarEl.textContent = UI.initials(`${user.firstName} ${user.lastName}`);
}
...
async function initApp() {
  ...
  if (Auth.isLoggedIn()) {
    buildSidebar();
    const hash = window.location.hash.slice(1);
    const startPage = hash && Router.routes[hash] ? hash
        ...
        if (results.totalResults > 0) {
          ...
          Object.entries(results.results).forEach(([mod, items]) => {
            ...
            items.forEach(item => {
              ...
              else html += '<div style="width:36px;height:36px;border-radius:50%;background:var(--accent);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:14px">' + (item.label||'?').charAt(0) + '</div>';
