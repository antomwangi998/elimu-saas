  ...
  async request(method, path, data = null, options = {}) {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        return { error: 'Server is taking too long to respond. It may be starting up — please try again in 30 seconds.' };
      }
...
var Router = {
  ...
  go(name, params = {}) {
    ...
    if (name !== 'login' && name !== 'landing' && !Auth.isLoggedIn()) {
  ...
  _showPage(name) {
    document.querySelectorAll('.page-view').forEach(el => {
      ...
      el.style.display = '';   // clear any inline display set by our landing-show script
    });
    const page = document.getElementById('page-' + name);
    if (page) page.classList.add('active');

    // Hide app shell on landing/login pages
    const noShell = name === 'landing' || name === 'login';
    const shell = document.getElementById('app-shell');
    if (shell) shell.style.display = noShell ? 'none' : '';
    // Also hide top header and sidebar individually (different selectors for safety)
    const topHeader = document.querySelector('.top-header') || document.querySelector('header');
    const sidebar   = document.querySelector('.sidebar');
    if (topHeader) topHeader.style.display = noShell ? 'none' : '';
...
var Toast = {
  ...
  success: (msg, title) => {
    if (window.NotifSound) NotifSound.play('success');
    if (msg && window.Confetti && /payment|paid|fee received/i.test(String(msg))) {
      Confetti.burst(window.innerWidth/2, window.innerHeight/3, 100);
    }
    return Toast.show(msg, 'success', title);
  },
  error: (msg, title) => { if(window.NotifSound) NotifSound.play('error'); return Toast.show(msg, 'error', title); },
...
function buildSidebar() {
  ...
  const sections = isSuperAdmin ? [
    ...
    { label: 'System', items: [
        { page: 'settings', icon: Icons.settings, label: 'Platform Settings' },
        { page: 'reset-passwords', icon: '🔑', label: 'Reset Passwords' },
    ]}
  ] : isStudent ? [
    ...
    },
    {
      ...
      items: [
        ...
        { page: 'report-cards', icon: '📋', label: 'Report Cards' },
        { page: 'kcse-prediction', icon: '🎯', label: 'KCSE Prediction' },
        { page: 'attendance', icon: Icons.attendance, label: 'Attendance' },
        { page: 'timetable', icon: '📅', label: 'Timetable' },
    ...
    },
    {
      label: 'Admissions',
      items: [
        { page: 'admissions', icon: '🏫', label: 'Admissions Pipeline' },
      ]
    },
    {
      label: 'Finance',
    ...
    ...(['dean_of_studies','principal','school_admin'].includes(role) ? [{ label: 'Admin', items: [
        { page: 'tsc-verification', icon: '🔐', label: 'TSC Verification' },
        { page: 'school-profile', icon: '🏫', label: 'School Profile' },
        { page: 'reset-passwords', icon: '🔑', label: 'Reset Staff Passwords' },
    ]}] : []),
    {
      label: 'System',
      items: [
        { page: 'settings', icon: Icons.settings, label: 'School Settings' },
      ]
    },
  ];
...
async function initApp() {
  ...
  if (Auth.isLoggedIn()) {
    ...
    Router.go('landing');
    // Trigger animations after landing is visible
    setTimeout(function() {
    ...
    overlay.innerHTML = `
      <div style="max-width:480px;text-align:center;padding:40px;background:white;border-radius:20px;box-shadow:0 8px 40px rgba(0,0,0,0.12)">
        ...
        <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
          <a href="mailto:elimusaas@gmail.com?subject=Account Renewal - ${AppState.user?.school_name||'School'}" 
        ...
        <div style="margin-top:20px;font-size:12px;color:#999">
          Support: elimusaas@gmail.com | Tel: 0729 612 106
        </div>
      </div>`;
...
// ── Animation Utilities ────────────────────────────────────────
window.Anim = {
  // Animated number counter (Zeraki-style)
  counter(el, target, duration = 1200, prefix = '', suffix = '') {
    if (!el) return;
    ...
    const tick = (now) => {
      ...
      const value = Math.round(start + range * easeOut(progress));
      el.textContent = prefix + value.toLocaleString('en-KE') + suffix;
  ...
  // Animate all .stat-value elements on a page
  animateStats(container = document) {
    container.querySelectorAll('.stat-value').forEach(el => {
      const text = el.textContent.trim();
      const num  = parseFloat(text.replace(/[^\d.]/g,''));
      if (!isNaN(num) && num > 0) {
        const prefix = text.match(/^[^0-9]*/)?.[0] || '';
        const suffix = text.match(/[^0-9%KM]*$/)?.[0] || '';
        Anim.counter(el, num, 1000, prefix, suffix);
      }
    });
  },
