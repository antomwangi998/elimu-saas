...
const getPayments = async (req, res) => {
  try {
    ...
    if (method)  { sql += ` AND fp.payment_method=$${p++}`; params.push(method); }
  ...
  try {
    const { rows: summary } = await query(`
      SELECT COALESCE(SUM(fp.amount) FILTER (WHERE fp.status='completed'),0) as total_collected,
             ...
             COALESCE(SUM(fp.amount) FILTER (WHERE fp.status='completed' AND fp.payment_method ILIKE '%mpesa%'),0) as mpesa_amount,
             COALESCE(SUM(fp.amount) FILTER (WHERE fp.status='completed' AND fp.payment_method='cash'),0) as cash_amount,
             COALESCE(SUM(fp.amount) FILTER (WHERE fp.status='completed' AND fp.payment_method='bank_transfer'),0) as bank_amount
