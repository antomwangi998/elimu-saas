...
const forgotPassword = async (req, res) => {
  ...
  try {
    const { rows } = await query(
      'SELECT id, first_name, phone FROM users WHERE email=$1 AND is_active=true', [email]);
    ...
    await cache.set(`pwd_reset:${resetToken}`, { userId: user.id, otp }, 3600);
    await cache.set(`pwd_otp:${email}`, { userId: user.id, otp, token: resetToken }, 7200);

    const resetUrl = `${process.env.FRONTEND_URL || 'https://elimu-saas-frontend.onrender.com'}/#reset?token=${resetToken}`;

    // Try to send email — never crash if SMTP is not configured
    sendEmail({
      to: email,
      ...
      html: `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:32px">
        ...
        <p>Hi ${user.first_name}, your OTP code is:</p>
        <div style="font-size:32px;font-weight:900;color:#1565C0;letter-spacing:6px;padding:16px;background:#EFF6FF;border-radius:10px;text-align:center">${otp}</div>
        <p style="margin-top:16px">Or use this link: <a href="${resetUrl}">Reset Password</a></p>
        <p style="color:#888;font-size:12px">Expires in 1 hour. ElimuSaaS.</p>
      </div>`,
    }).catch(err => logger.warn('Forgot password email failed:', err.message));
    ...
    logger.error('forgotPassword error:', e.message);
