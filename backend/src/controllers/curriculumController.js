...
const seedKnecScaleHandler = async (req, res) => {
  try {
    const { withTransaction } = require('../config/database');
    await withTransaction(async (client) => {
      await seedKnecScale(req.schoolId, client);
    });
    res.json({ success: true, message: 'KNEC scale seeded' });
...
module.exports = {
  ...
  getFullReportCard,
  getKnecScale, updateKnecScale, seedKnecScale: seedKnecScaleHandler,
