...
async function startServer() {
  try {
    ...
    try {
      ...
      const count = parseInt(schoolCount[0].count);
      if (count < 5) {
        logger.info(`📊 Only ${count} schools found — running demo seed...`);
        const seedFn = require('./migrations/seed_schools');
        if (typeof seedFn === 'function') {
          await seedFn();
          logger.info('✅ Demo schools seeded!');
        }
      } else {
        logger.info(`✅ ${count} schools already in DB — skipping seed`);
      }
    } catch (seedErr) {
