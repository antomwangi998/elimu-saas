...
app.post('/api/admissions/online', async (req, res) => {
  try {
    const { query } = require('./config/database');
    const { firstName, lastName, dateOfBirth, gender, applyingFor, kcpeScore,
            parentName, parentPhone, schoolName, notes } = req.body;
    if (!firstName || !lastName || !parentPhone) {
      return res.status(400).json({ error: 'First name, last name and parent phone required' });
    }
    // Find school by name if provided
    let schoolId = null;
    if (schoolName) {
      const { rows } = await query("SELECT id FROM schools WHERE name ILIKE $1 LIMIT 1", [`%${schoolName}%`]);
      schoolId = rows[0]?.id || null;
    }
    // Store in a generic table (using audit_logs as a lightweight store)
    const { rows } = await query(
      `INSERT INTO audit_logs(school_id,action,entity_type,new_data,ip_address)
       VALUES($1,'online_admission','application',$2,$3) RETURNING id`,
      [schoolId, JSON.stringify({firstName,lastName,dateOfBirth,gender,applyingFor,kcpeScore,parentName,parentPhone,notes}), req.ip||null]);
    res.status(201).json({ message: 'Application received', id: rows[0]?.id, reference: 'ADM-'+Date.now() });
...
app.use('/api/otp', otpRoutes); // public - no auth middleware
...
// ── 404 Handler ───────────────────────────────────────────────
// ── Seed trigger (GET or POST, requires secret) ──────────────
app.all('/api/trigger-seed', async (req, res) => {
  const secret = req.headers['x-seed-secret'] || req.query.secret;
  const expected = process.env.SEED_SECRET || 'elimusaas-seed-2024';
  if (!secret || secret !== expected) {
    return res.status(403).json({ error: 'Invalid secret. Pass ?secret=elimusaas-seed-2024' });
  }
  res.json({ message: '🌱 Seed running in background — check Render logs in ~2 mins' });
  setImmediate(async () => {
    try {
      logger.info('🌱 Manual seed triggered via HTTP...');
      const seedFn = require('./migrations/seed_schools');
      await seedFn();
...
async function startServer() {
  try {
    ...
    // Auto-seed demo schools if fewer than 5 schools exist
    try {
      const { query: dbQuery } = require('./config/database');
      const { rows: schoolCount } = await dbQuery('SELECT COUNT(*) FROM schools');
      const { rows: studentCount } = await dbQuery('SELECT COUNT(*) FROM students');
      const numSchools  = parseInt(schoolCount[0].count);
      const numStudents = parseInt(studentCount[0].count);
      if (numSchools < 5 || numStudents < 500) {
        logger.info(`📊 ${numSchools} schools, ${numStudents} students — running seed...`);
        const seedFn = require('./migrations/seed_schools');
        ...
        logger.info(`✅ ${numSchools} schools, ${numStudents} students in DB`);
      }
    } catch (seedErr) {
      logger.warn('⚠️  School seed skipped:', seedErr.message);
    }
